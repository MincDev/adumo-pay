...
protocol InitiateTransactionUseCase {
    func execute(with transaction: Transaction, authenticatedWith authData: AuthData) async throws -> TransactionData
}
...
final class InitiateTransactionUseCaseImpl: InitiateTransactionUseCase {
    ...
    func execute(with transaction: Transaction, authenticatedWith authData: AuthData) async throws -> TransactionData {
        try await transRepo.initiate(with: transaction, authenticatedWith: authData)
    }
}
